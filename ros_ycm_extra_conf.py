...
#                                                                        #
# https://gist.github.com/galou/92a2d05dd772778f86f2                     #
#                                                                        #
# Name this file .ycm_extra_conf.py and place it in your ROS workspace   #
# to use it.                                                             #
#                                                                        #
# Tested with Debian Stretch and kinetic.                                #
#                                                                        #
# License: CC0                                                           #
##########################################################################
...
# change this to fit your workspace location
# ROS_WORKSPACE = '/path/to/your/catkin_ws'
ROS_WORKSPACE = os.path.dirname(os.path.abspath(__file__))

def SetEnvVars():
    """Set ROS environment variables (if they're not set already)
    """
    # don't clobber existing environment variables, but set them
    # if they're unset (in case emacs wasn't started from a shell with setup.bash already sourced
    if 'ROS_ROOT' not in os.environ:
        os.environ['ROS_ROOT'] = '/opt/ros/kinetic/share/ros'

    if 'ROS_PACKAGE_PATH' not in os.environ:
        os.environ['ROS_PACKAGE_PATH'] = ROS_WORKSPACE + '/src:/opt/ros/kinetic/share:/opt/ros/kinetic/stacks'

    if 'ROSLISP_PACKAGE_DIRECTORIES' not in os.environ:
        os.environ['ROSLISP_PACKAGE_DIRECTORIES'] = ROS_WORKSPACE + '/devel/share/common-lisp'
    ...
    os.environ['PYTHONPATH'] = ROS_WORKSPACE + '/devel/lib/python2.7/dist-packages:/opt/ros/kinetic/lib/python2.7/dist-packages' + (os.environ['PYTHONPATH'] if 'PYTHONPATH' in os.environ else '')
...
def GetRosIncludePaths():
    ...
    rospack = RosPack()
    includes = []
    includes.append(ROS_WORKSPACE + '/devel/include')
...
# in your shell (if you use catkin tools).
# If you don't use catkin tools, enter
...
def GetCompilationDatabaseFolder(filename):
    ...
    # if there's a workspace-wide compile_commands.json, use that!
    if(os.path.isfile(ROS_WORKSPACE + '/build/compile_commands.json')):
        return ROS_WORKSPACE + '/build'
...
def GetDatabase(compilation_database_folder):
    if os.path.exists(compilation_database_folder):
        # moved import here to be able to test the other functions
        import ycm_core
